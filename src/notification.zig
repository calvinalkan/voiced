...
pub fn init(self: *Notification, epoll_fd: std.posix.fd_t, tag: u64, bus_address: ?[]const u8, runtime_directory: ?[]const u8) void {
    self.configure(bus_address, runtime_directory) catch |err| {
        log.event(.warn, .{}, "notification_unavailable", &.{
...
pub fn initEmpty(self: *Notification) void {
    ...
    inline for (std.meta.fields(Notification)) |field| {
        if (comptime std.mem.eql(u8, field.name, "connection")) {
            ...
            @field(self, field.name) = comptime field.defaultValue() orelse @compileError("Notification metadata requires a default: " ++ field.name);
        }
    }
}
...
fn process(self: *Notification, epoll_fd: std.posix.fd_t, tag: u64) !void {
    ...
    if (now >= self.operation_deadline_ns) {
        ...
        log.event(.warn, .{}, "notification_request_timed_out", &.{
            .{ "operation", .{ .name = @tagName(std.meta.activeTag(request.operation.?)) } },
            .{ "bus_error", .{ .str = "org.freedesktop.DBus.Error.NoReply" } },
            .{ "detail", .{ .str = "reply deadline exceeded" } },
            .{ "serial", .{ .u = request.serial } },
        });
        ...
        if (self.phase == .authenticating) {
            ...
            if (std.mem.indexOf(u8, bytes, "\r\n")) |end| {
                ...
                if (line.len != 35 or !std.mem.startsWith(u8, line, "OK ")) {
                    log.event(if (self.transport_problem_reported) .debug else .warn, .{}, "notification_authentication_rejected", &.{
                        .{ "response", .{ .str = line } },
                    });
...
fn received(self: *Notification, message: dbus.Message) !void {
    ...
    if (message.kind == .signal) {
        if (std.mem.eql(u8, message.sender, bus_destination) and std.mem.eql(u8, message.path, bus_path) and std.mem.eql(u8, message.interface, bus_destination) and std.mem.eql(u8, message.member, "NameOwnerChanged")) {
            ...
            log.event(.debug, .{}, "notification_owner_changed", &.{
                .{ "owner_previous", .{ .str = previous } },
                .{ "owner_next", .{ .str = next } },
            });
            ...
            if (id == notification.id) {
                self.notification = null;
                log.event(.debug, .{}, "notification_closed", &.{
                    .{ "notification_id", .{ .u = id } },
                    .{ "reason", .{ .u = reason } },
                });
            }
        }
    ...
    if (message.kind == .err) {
        ...
        log.event(.warn, .{}, "notification_request_failed", &.{
            .{ "operation", .{ .name = if (request.operation) |op| @tagName(op) else @tagName(self.phase) } },
            .{ "bus_error", .{ .str = message.error_name } },
            .{ "detail", .{ .str = detail } },
            .{ "serial", .{ .u = message.reply_serial } },
        });
        self.notification = null;
    ...
    switch (self.phase) {
        ...
        .match_closed, .match_owner => {
            ...
            if (self.phase == .ready) {
                ...
                log.event(.debug, .{}, "notification_bus_ready", &.{
        ...
        .ready => switch (request.operation.?) {
            .show => |problem| {
                ...
                if (id == 0 or from_bus) {
                    self.notification = null;
                    log.event(.warn, .{}, "notification_reply_invalid", &.{});
                    return;
                }
                try body.end();
                self.notification = .{ .id = id, .owner = try Name.init(message.sender) };
                log.event(.debug, .{}, "notification_accepted", &.{
                    .{ "problem", .{ .name = @tagName(problem.problem) } },
                });
            },
            .close => {
                if (message.signature.len != 0) return error.InvalidCloseReply;
                try body.end();
                log.event(.debug, .{}, "notification_close_accepted", &.{});
            },
        },
...
fn disconnected(self: *Notification, epoll_fd: std.posix.fd_t, err: anyerror) void {
    ...
    log.event(if (repeated) .debug else .warn, .{}, "notification_transport_failed", &.{
        .{ "outcome", .{ .name = if (established) "disconnected" else if (repeated) "retry_failed" else "unavailable" } },
        .{ "error", .{ .verbatim = @errorName(err) } },
        .{ "phase", .{ .name = @tagName(self.phase) } },
        .{ "system_error", .{ .errno = self.connection.errno } },
        .{ "retry_duration_seconds", .{ .u = 5 } },
    });
    self.transport_problem_reported = true;
    self.close(epoll_fd);
    self.request = null;
