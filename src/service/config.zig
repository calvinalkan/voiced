...
pub fn load(init: std.process.Init, arguments: []const [:0]const u8, diagnostic: *Diagnostic) !Supervisor.ServiceOptions {
    ...
    if (path) |file_path| {
        ...
        const text = std.Io.Dir.cwd().readFileAlloc(init.io, file_path, allocator, .limited(64 * 1024)) catch |err| text: {
            if (err == error.FileNotFound and explicit_path == null) {
                break :text "";
            }
        ...
        while (lines.next()) |raw_line| {
            ...
            const option = std.meta.stringToEnum(Option, name) orelse {
...
fn set(allocator: std.mem.Allocator, options: *Supervisor.ServiceOptions, seen: *Seen, option: Option, value: []const u8, diagnostic: *Diagnostic) !void {
    ...
    switch (option) {
        ...
        .log_level => {
            ...
            options.log_level = logging.parseLevel(value) orelse {
                return error.InvalidValue;
            };
        },

        .log_target => {
            diagnostic.expected = "'auto', 'journal', or 'stderr'";
            options.log_target = logging.parseTarget(value) orelse {
                return error.InvalidValue;
            };
        },

        .model => {
            diagnostic.expected = "'whisper.base.en', 'whisper.small.en', 'whisper.medium.en', or 'whisper.tiny.en'";
            options.capture.transcription.model = inference.Model.parseKind(value) orelse {
                return error.InvalidValue;
            };
        },

        .model_encoder_threads, .model_decoder_threads => {
            diagnostic.expected = "a positive integer number of worker threads";

            const count = std.fmt.parseInt(usize, value, 10) catch {
                return error.InvalidValue;
            };
        ...
        .model_encoder_padding_seconds => {
            ...
            const seconds = std.fmt.parseInt(u8, value, 10) catch {
                return error.InvalidValue;
            };
        ...
        .model_idle_seconds_max => {
            ...
            options.model_keep_warm_seconds = std.fmt.parseInt(u32, value, 10) catch {
                return error.InvalidValue;
            };
        },
        ...
        .recording_seconds_max => {
            ...
            const seconds = std.fmt.parseInt(u16, value, 10) catch {
                return error.InvalidValue;
            };
        ...
        .microphone_node, .microphone_serial => {
            ...
            if (value.len == 0 or value.len >= Capture.target_name_bytes_capacity or std.mem.indexOfScalar(u8, value, 0) != null) {
                return error.InvalidValue;
            }

            const terminated = try allocator.dupeZ(u8, value);

            options.capture.source = if (option == .microphone_node)
                .{ .node_name = terminated }
            else
        ...
        .transcript_output => {
            ...
            options.output = std.meta.stringToEnum(@FieldType(Supervisor.ServiceOptions, "output"), value) orelse {
                return error.InvalidValue;
            };
        },

        .clipboard_backend => {
            diagnostic.expected = "'auto', 'wayland', or 'x11'";
            options.clipboard_backend = std.meta.stringToEnum(@FieldType(Supervisor.ServiceOptions, "clipboard_backend"), value) orelse {
                return error.InvalidValue;
            };
        },
        ...
        .notification_mode => {
            ...
            options.notification_mode = std.meta.stringToEnum(@FieldType(Supervisor.ServiceOptions, "notification_mode"), value) orelse {
                return error.InvalidValue;
            };
        },

        .paste_settle_ms, .paste_key_gap_ms, .paste_observation_ms => {
            diagnostic.expected = "an integer from 0 to 65535 milliseconds";

            const milliseconds = std.fmt.parseInt(u16, value, 10) catch {
                return error.InvalidValue;
            };

            switch (option) {
                .paste_settle_ms => options.paste_settle_ms = milliseconds,
                .paste_key_gap_ms => options.paste_key_gap_ms = milliseconds,
                .paste_observation_ms => options.paste_observation_ms = milliseconds,
                else => unreachable,
            }
        },
        ...
        .paste_shortcut => {
            ...
            options.paste_key = std.meta.stringToEnum(Paste.Chord, value) orelse {
                return error.InvalidValue;
            };
        },
    }
...
const Arguments = struct {
    ...
    fn next(arguments: *Arguments, diagnostic: *Diagnostic) !?struct { option: Option, value: []const u8 } {
        ...
        const option = cliOption(name) orelse {
            return error.UnknownOption;
        };
