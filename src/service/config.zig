...
fn set(allocator: std.mem.Allocator, options: *Supervisor.ServiceOptions, seen: *Seen, option: Option, value: []const u8, diagnostic: *Diagnostic) !void {
    ...
    switch (option) {
        ...
        .log_target => {
            ...
            options.log_target = logging.parseTarget(value) orelse return error.InvalidValue;
        },
        .model => {
            diagnostic.expected = "'whisper.base.en', 'whisper.small.en', or 'whisper.medium.en'";
            options.capture.transcription.model = inference.Model.parseKind(value) orelse return error.InvalidValue;
        },
        .model_encoder_threads, .model_decoder_threads => {
            diagnostic.expected = "a positive integer number of worker threads";
            const count = std.fmt.parseInt(usize, value, 10) catch return error.InvalidValue;
        ...
        .clipboard_backend => {
            ...
            options.clipboard_backend = std.meta.stringToEnum(@FieldType(Supervisor.ServiceOptions, "clipboard_backend"), value) orelse return error.InvalidValue;
        },
        .notification_mode => {
        ...
        .paste_shortcut => {
            ...
            options.paste_key = std.meta.stringToEnum(Paste.Chord, value) orelse return error.InvalidValue;
        },
    }
}
