...
const RuntimeDirectoryUnavailable = log.Event(.runtime_directory_unavailable, DiagnosticFields);
const ControlBindFailed = log.Event(.control_bind_failed, DiagnosticFields);
const ControlCleanupFailed = log.Event(.control_cleanup_failed, DiagnosticFields);
const ControlListenFailed = log.Event(.control_listen_failed, DiagnosticFields);
const ControlShutdownFailed = log.Event(.control_shutdown_failed, DiagnosticFields);
const ControlAcceptFailed = log.Event(.control_accept_failed, DiagnosticFields);
const ControlReplyFailed = log.Event(.control_reply_failed, DiagnosticFields);
const RuntimeDirectoryInspectionFailed = log.Event(.runtime_directory_inspection_failed, DiagnosticFields);
const RuntimeDirectoryInvalid = log.Event(.runtime_directory_invalid, DiagnosticFields);
const ControlSocketFailed = log.Event(.control_socket_failed, DiagnosticFields);
const ControlRegistrationFailed = log.Event(.control_registration_failed, DiagnosticFields);
...
comptime {
    if (builtin.target.cpu.arch.endian() != .little) {
        @compileError("control wire ABI requires little endian");
    }

    assert(@sizeOf(WireRequest) == 8);

    for (std.meta.fields(WireRequest), [_]usize{ 0, 4, 6, 7 }) |field, offset| {
        assert(@offsetOf(WireRequest, field.name) == offset);
    }

    assert(@sizeOf(WireReply) == 64);

    for (std.meta.fields(WireReply), [_]usize{ 0, 4, 6, 7, 8, 16, 24, 32, 40, 48, 49, 50, 51 }) |field, offset| {
        assert(@offsetOf(WireReply, field.name) == offset);
    }

    for (std.meta.fields(@FieldType(Request, "cmd"))) |field| {
        assert(field.value == @intFromEnum(@field(WireRequest.Command, field.name)));
    }
}
...
pub const Server = struct {
    ...
    pub fn open(init: std.process.Init, epoll_fd: std.posix.fd_t, listener_tag: u64, client_tag: u64) !Server {
        ...
        const runtime_dir = std.Io.Dir.cwd().openDir(init.io, runtime_root, .{}) catch |err| {
            RuntimeDirectoryUnavailable.emit(.err, .{}, .{
                .path = runtime_root,
                .@"error" = @errorName(err),
            });
        ...
        switch (linux.errno(bind_result)) {
            ...
            .ADDRINUSE => {
                ...
                if (linux.errno(retry_result) != .SUCCESS) {
                    ControlBindFailed.emit(.err, .{}, .{
                        .operation = "bind",
                        .system_error = linux.errno(retry_result),
                    });
                ...
                ControlBindFailed.emit(.err, .{}, .{
                    .operation = "bind",
                    .system_error = linux.errno(bind_result),
                });
        ...
        errdefer directory.deleteFile(init.io, "control.sock") catch |err| {
            if (err != error.FileNotFound) ControlCleanupFailed.emit(.err, .{}, .{
                .operation = "unlink",
                .@"error" = @errorName(err),
            });
        };
        ...
        if (linux.errno(listen_result) != .SUCCESS) {
            ControlListenFailed.emit(.err, .{}, .{
                .operation = "listen",
                .system_error = linux.errno(listen_result),
            });
    ...
    pub fn deinit(server: *Server, io: std.Io) void {
        for (0..server.clients.len) |index| {
    ...
    pub fn stopAccepting(server: *Server) !void {
        ...
        if (errno != .SUCCESS) {
            ControlShutdownFailed.emit(.err, .{}, .{
    ...
    pub fn deadline(server: *const Server) ?u64 {
        ...
        for (&server.clients) |*client| {
            if (client.descriptor != null) {
                earliest = @min(earliest orelse
                    std.math.maxInt(u64), client.deadline_ns);
            }
        }
    ...
    pub fn expire(server: *Server, now_ns: u64) void {
        for (&server.clients, 0..) |*client, index| {
            if (client.descriptor != null and now_ns >= client.deadline_ns) {
                server.closeClient(index);
            }
        }
    }
    ...
    pub fn hasPendingReplies(server: *const Server) bool {
        for (&server.clients) |*client| {
            if (client.descriptor != null and client.response != null) {
                return true;
            }
        }
    ...
    fn flush(server: *Server, index: usize) void {
        ...
        while (true) {
            ...
            switch (linux.errno(result)) {
                ...
                .SUCCESS => {
                    if (result != bytes.len) ControlReplyFailed.emit(.err, .{}, .{
                        .reason = "short_write",
                        .write_size = result,
                ...
                .AGAIN => {
                    ...
                    if (linux.errno(linux.epoll_ctl(server.epoll_fd, linux.EPOLL.CTL_MOD, descriptor, &event)) != .SUCCESS) {
                        server.closeClient(index);
                    }
...
pub fn sendRequest(init: std.process.Init, request: Request) !void {
    ...
    while (true) {
        ...
        if (linux.errno(result) != .SUCCESS or result != @sizeOf(WireRequest)) {
    ...
    if (reply_size != @sizeOf(WireReply)) {
        return error.InvalidControlReply;
    }

    if (!std.mem.eql(u8, &wire_reply.magic, "VCDR")) {
        return error.InvalidControlReply;
    }

    if (wire_reply.version != 1) {
        return error.IncompatibleControlProtocol;
    }

    if (!std.mem.allEqual(u8, &wire_reply.reserved, 0)) {
        return error.InvalidControlReply;
    }

    if (wire_reply.command != wire.command) {
        return error.InvalidControlReply;
    }
    ...
    switch (wire_reply.result) {
        .accepted, .ignored => {
            if (wire_reply.result == .ignored and request.cmd != .record) {
                return error.InvalidControlReply;
            }
            ...
            switch (wire_reply.model_kind) {
                .whisper_base_en, .whisper_small_en, .whisper_medium_en, .whisper_tiny_en => {},
                else => return error.InvalidControlReply,
            }

            if (wire_reply.model_idle_seconds_max > std.math.maxInt(u32)) {
                return error.InvalidControlReply;
            }

            if (recording_elapsed_seconds != null and wire_reply.phase != .capturing) {
                return error.InvalidControlReply;
            }
            ...
            if (recording_elapsed_seconds) |elapsed| {
                if (elapsed > wire_reply.daemon_uptime_seconds) {
                    return error.InvalidControlReply;
                }
            }
        },
...
fn requireOwnedPrivateDirectory(handle: std.posix.fd_t, path: []const u8) !void {
    ...
    if (stat_errno != .SUCCESS) {
        RuntimeDirectoryInspectionFailed.emit(.err, .{}, .{
            .path = path,
            .operation = "statx",
            .system_error = stat_errno,
        });
    ...
    if (!stat.mask.TYPE or stat.mode & linux.S.IFMT != linux.S.IFDIR) {
        RuntimeDirectoryInvalid.emit(.err, .{}, .{
    ...
    if (!stat.mask.UID or !stat.mask.MODE or stat.uid != linux.geteuid() or stat.mode & 0o077 != 0) {
        RuntimeDirectoryInvalid.emit(.err, .{}, .{
            ...
            .path = path,
            .uid = stat.uid,
            .uid_expected = linux.geteuid(),
            .mode = stat.mode & 0o777,
            .uid_available = stat.mask.UID,
            .mode_available = stat.mask.MODE,
        });
...
fn createSocket(nonblocking: bool) !std.posix.fd_t {
    const result = linux.socket(linux.AF.UNIX, linux.SOCK.SEQPACKET | linux.SOCK.CLOEXEC | (if (nonblocking)
        @as(u32, linux.SOCK.NONBLOCK)
    else
        0), 0);

    if (linux.errno(result) != .SUCCESS) {
        ControlSocketFailed.emit(.err, .{}, .{
            .operation = "socket",
            .system_error = linux.errno(result),
        });
...
fn register(epoll_fd: std.posix.fd_t, descriptor: std.posix.fd_t, tag: u64, events: u32) !void {
    ...
    if (linux.errno(result) != .SUCCESS) {
        ControlRegistrationFailed.emit(.err, .{}, .{
...
fn close(descriptor: std.posix.fd_t) void {
    ...
    if (errno != .SUCCESS) ControlCleanupFailed.emit(.err, .{}, .{
        .operation = "close",
        .descriptor = descriptor,
        .system_error = errno,
    });
