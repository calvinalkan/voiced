...
const memory_alignment: usize = std.atomic.cache_line;
const encoder_features_offset: usize = std.mem.alignForward(usize, fft_bins_count, memory_alignment / @sizeOf(f32));
...
pub fn workspaceValuesCount(encoder_positions_count: usize) usize {
    assert(encoder_positions_count > 0);
    assert(encoder_positions_count <= encoder_frames_count_max / 2);

    return encoder_features_offset + mel_bins_count * encoder_positions_count * 2;
}
...
pub const EncoderTrailingPadding = abi.Padding;
...
pub const Extractor = struct {
    ...
    pub const memory_size: usize = @sizeOf(Tables);
    ...
    pub fn init(extractor: *Extractor, memory: []align(memory_alignment) u8, configured_samples_count_max: usize) error{MemoryTooSmall}!void {
        ...
        // Fill caller storage in place. Assigning a default-initialized Tables
        // aggregate could embed its entire coefficient storage in the binary.
        const tables: *Tables = @ptrCast(memory.ptr);
        const hann_window = &tables.hann_window;
        const dft_cosines = &tables.dft_cosines;
        const dft_sines = &tables.dft_sines;
        const mel_filters = &tables.mel_filters;
        const mel_filter_coefficients = &tables.mel_filter_coefficients;
    ...
    /// 30-second context. `workspace` may alias neither the samples nor the
    ...
    pub fn calculate(extractor: *const Extractor, samples: []const f32, trailing_padding: EncoderTrailingPadding, workspace: []f32) Error!Features {
        ...
        for (0..content_frames_count) |frame_index| {
            ...
            calculatePowerSpectrum(power_spectrum, frame_samples, &extractor.tables.hann_window, &extractor.tables.dft_cosines, &extractor.tables.dft_sines);
            calculateMelFrame(encoder_features, encoder_frames_count, frame_index, power_spectrum, &extractor.tables.mel_filters, &extractor.tables.mel_filter_coefficients);
        }
...
fn paddedEncoderFramesCount(content_frames_count: usize, trailing_padding: EncoderTrailingPadding) usize {
    ...
    const requested_frames_count = content_frames_count + @as(usize, trailing_padding.seconds()) * frames_per_second;
...
// Coefficient sizes depend only on Whisper's fixed 400-sample DFT and 80
// Mel bins. Audio duration changes the separate calculation workspace only.
const Tables = struct {
    hann_window: [fft_samples_count]f32 align(memory_alignment),
    dft_cosines: [fft_bins_count * fft_samples_count]f32 align(memory_alignment),
    dft_sines: [fft_bins_count * fft_samples_count]f32 align(memory_alignment),
    mel_filters: [mel_bins_count]MelFilter align(memory_alignment),
    mel_filter_coefficients: [mel_filter_coefficients_capacity]f32 align(memory_alignment),
};
