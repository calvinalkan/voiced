...
/// Every collective lane must participate; callers synchronize after return.
...
/// from all input/output storage and other lanes. Every collective lane must
...
/// Adds one projected row directly into `output`. Input, output, and each
/// lane's scratch must be disjoint. Callers synchronize after return.
pub fn forwardResidualOne(input: []const f32, weight: QuantizedWeight, bias: []const f32, output: []f32, quantized_scratch: []u8, lane: Lane) void {
    validateProjection(input, 1, weight, bias, output);
    assert(quantized_scratch.len >= weight.input_values_count);
...
fn projectOneOutputRange(comptime add_to_output: bool, quantized_input: []const u8, input_scale: f32, weight: QuantizedWeight, bias: []const f32, activation: Activation, output_block_begin: usize, output_block_end: usize, output: []f32) void {
...
/// row tiles and disjoint scratch; every collective lane must participate.
...
// integer dot-product kernel keeps its compile-time register tiles. Inlining
// these row loops duplicated scaling, bias, transposition, and output stores.
// Sharing them saved 13.2 KiB in the stripped static Zig 0.16/LLVM binary
// (application/stdlib ReleaseSmall, inference ReleaseFast).
//
// On an i7-13700HX with four workers, eight clips per model (Base.en/Small.en,
...
noinline fn finishDirectLayoutTile(comptime output_layout: ProjectionOutputLayout, accumulators: []const I32x8, input_scales: []const f32, weight: QuantizedWeight, bias: []const f32, activation: Activation, output_block_begin: usize, output_row_begin: usize, positions_capacity: usize, output: anytype) void {
    ...
    if (output_layout == .encoder_query_key_value or rows_count == simd_lanes_count) {
        ...
        if (directOutputColumnIsPackedKey(output_layout, output_begin, weight.scales.len)) {
            ...
            for (projected_rows[0..rows_count], 0..) |values, tile_row| {
...
noinline fn calculateProjectionAccumulators(comptime rows_count: usize, comptime output_blocks_count: usize, quantized_rows: []const u8, weight: QuantizedWeight, output_block_begin: usize) [rows_count][output_blocks_count]I32x8 {
    ...
    // PERFORMANCE: Each packed group pairs four activation bytes with 32
    // weight bytes: four input depths for eight output rows. Check complete
    // tiles first, then give every row and block exactly groups_count
    // fixed-size elements. Iterating over 0..groups_count lets LLVM prove
    // each group access is in bounds and eliminate per-load checks. Checks
    // before the loop remain enabled in ReleaseSafe. The views borrow the
    // existing storage without copying or repacking.
    //
    // A benchmarked alternative computed each weight address from a block
    // base plus eight times the input-depth index, then took a 32-byte slice.
    // For that implementation, Zig 0.16/LLVM retained overflow and bounds
    // checks inside the depth loop. Enabling those checks made full Small.en
    // transcription about 20% slower than disabling them in that same loop
    // (i7-13700HX, four workers, 10-second padding).
    //
    // Preserve the equal-length group views if the layout or callers change.
    // Recheck ReleaseSafe assembly and full transcription timings before
    // replacing them with per-load byte-offset calculations.
    const groups_count = @divExact(weight.input_values_count, depth_values_per_group);
    const activation_storage_bytes = quantized_rows[0 .. rows_count * weight.input_values_count];
    const activation_groups = std.mem.bytesAsSlice([depth_values_per_group]u8, activation_storage_bytes);
...
// PERFORMANCE: Keep row-major finishing out of line with runtime tile sizes.
// Inlining and unrolling it for each integer tile shape duplicated scaling,
// bias, GELU, and residual stores, and made LLVM load and spill accumulators
// ahead of their use. This shared loop consumes one output block at a time;
// the integer kernel keeps its compile-time tile sizes and register scheduling.
//
// With Zig 0.16/LLVM on an i7-13700HX, sharing this finisher cut the whole
// stripped static production binary by 111 KiB (11.2%): application/stdlib
// ReleaseSmall, inference ReleaseFast. Eight LibriSpeech clips per model
// (Base.en and Small.en, 2.5-27 s, four workers, 10 s trailing padding) tested
// both that profile and an all-ReleaseSafe build. Retired instructions rose
// 0.7-1.1%; the median paired timing change across clips stayed within 0.4%
// in each model/profile. Individual clip medians ranged from -2.1% to +2.2%,
// with noisy runs repeated. This trades call/loop setup for shared code;
// these measurements establish no consistent transcription speedup or slowdown.
//
// Check whole rows and expose equal-length output and scale blocks before
// traversing each row. Recheck binary size and full transcription timings
// before inlining it.
noinline fn finishRowMajorTile(comptime add_to_output: bool, accumulators: []const I32x8, input_scales: []const f32, weight: QuantizedWeight, bias: []const f32, activation: Activation, output_block_begin: usize, output: []f32) void {
    const rows_count = input_scales.len;
    const output_blocks_count = @divExact(accumulators.len, rows_count);
    const output_begin = output_block_begin * output_rows_per_block;
...
// TODO: Revisit these compiler-backend fallbacks alongside future AVX2 support
// without VNNI. They are not CPU-feature dispatch.
