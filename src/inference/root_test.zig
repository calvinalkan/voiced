...
test "transcribes audio fixtures" {
    ...
    const workers_count = if (std.testing.environ.getPosix("VOICED_RUNTIME_WORKERS")) |value|
        try std.fmt.parseInt(usize, value, 10)
    else
        4;

    const decoder_workers_count = if (std.testing.environ.getPosix("VOICED_RUNTIME_DECODER_WORKERS")) |value|
        try std.fmt.parseInt(usize, value, 10)
    else
        null;
    ...
    const word_error_rate_max: f64 = switch (test_model.kind) {
        .whisper_base_en, .whisper_small_en, .whisper_medium_en => 0.06,
        .whisper_tiny_en => 0.10,
    };
    ...
    try std.testing.expect(word_error_rate <= word_error_rate_max);
}
...
test "token limits preserve complete UTF-8 characters and all tokens" {
    ...
    for (samples, 0..) |*sample, index| {
        sample.* = @as(f32, @floatFromInt(std.mem.readInt(i16, pcm[index * 2 ..][0..2], .little))) / 32768;
    }
...
test "object boundary supports a pool larger than one bitset word" {
    ...
    for (pool_memory[pool_size..]) |byte| {
...
test "object boundary shares a pool between concurrent runtimes" {
    ...
    const Context = struct {
        ...
        fn run(context: *@This()) void {
            while (!context.start.load(.acquire)) {
                std.atomic.spinLoopHint();
            }
    ...
    for (0..8) |round| {
        ...
        for (contexts) |context| {
            const result = context.outcome orelse {
    ...
    for (positions_range.start_index..positions_range.end_index) |position| {
        ...
        for (0..channels_count) |channel| {
            for (0..3) |kernel_index| {
                ...
                float_row[channel * 3 + kernel_index] = if (source < 0 or source >= input_positions_count)
                    0
                else
                    input[
                        if (channel_major)
                            channel * input_positions_count + @as(usize, @intCast(source))
                        else
                            @as(usize, @intCast(source)) * channels_count + channel
                    ];
            }
        }
...
test "benchmarks token projections" {
    ...
    const weight = if (std.mem.eql(u8, selected, "vocabulary"))
        weights.decoder_embeddings_weight
    else
        weights.decoder_layers[0].ffn_contraction_weight;
...
fn selectedTestModel() !TestModel {
    ...
    if (std.mem.eql(u8, name, "whisper.tiny.en")) {
        return .{ .name = name, .kind = .whisper_tiny_en };
    }
