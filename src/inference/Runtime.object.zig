...
pub fn transcribe(runtime: *Runtime, request: *const abi.Request) TranscriptionResult {
    if (request.encoder_padding.seconds() > runtime.encoder_padding_max.seconds()) return .encoder_padding_exceeds_limit;
    const samples = request.samples;
    const encoder_workers_count = request.encoder_workers_count_max orelse runtime.scheduler.workersCount();
    ...
    if (encoder_workers_count == 0 or encoder_workers_count > runtime.scheduler.workersCount() or
        decoder_workers_count == 0 or decoder_workers_count > runtime.scheduler.workersCount()) return .invalid_worker_count;
    ...
    if (samples.len < log_mel.samples_count_min) return .audio_too_short;
    if (samples.len > runtime.extractor.samples_count_max) return .audio_duration_exceeds_limit;
    for (samples) |sample| {
        if (!std.math.isFinite(sample) or sample < -1.0 or sample > 1.0) return .invalid_samples;
    }
    ...
    const features = runtime.extractor.calculate(samples, request.encoder_padding, runtime.encoder.query_key_value) catch unreachable;
    ...
    runtime.scheduler.run(decoder_workers_count, @ptrCast(&context), decodeStep, request.cancellation) catch {
        ...
        if (context.decoder_phase == .cross) timings.cross_key_values_ns = elapsed else timings.decoder_ns = elapsed;
        return .{ .cancelled = timings };
    };
    ...
    for (tokens) |token| {
        if (token >= end_of_text_token) continue;
    ...
    output.text = transcriptText(output.text, token_limit) catch return .{ .invalid_transcript_encoding = output };
    return if (token_limit) .{ .token_limit = output } else .{ .ok = output };
}
...
fn transcriptText(bytes: []const u8, token_limit: bool) error{InvalidTranscriptEncoding}![]const u8 {
    ...
    if (!token_limit) return error.InvalidTranscriptEncoding;
    var start = bytes.len;
    while (start > 0 and bytes[start - 1] & 0xc0 == 0x80) : (start -= 1) {}
    if (start == 0) return error.InvalidTranscriptEncoding;
    start -= 1;
    const size = std.unicode.utf8ByteSequenceLength(bytes[start]) catch return error.InvalidTranscriptEncoding;
    const tail = bytes[start..];
    if (tail.len >= size) return error.InvalidTranscriptEncoding;
    ...
    _ = std.unicode.utf8Decode(completed[0..size]) catch return error.InvalidTranscriptEncoding;
    if (!std.unicode.utf8ValidateSlice(bytes[0..start])) return error.InvalidTranscriptEncoding;
...
fn decodeStep(raw_context: *anyopaque, lane: Lane) Scheduler.Step {
        ...
        .cross => |layer_index| {
            ...
            if (lane.isLeader()) {
                if (layer_index + 1 == dimensions.decoder_layers_count) {
                    ...
                    context.phase_start = now;
                } else context.decoder_phase = .{ .cross = layer_index + 1 };
            }
            return .more;
        },
        .prompt_start => {
    ...
    if (lane.isLeader()) {
        ...
        if (selection.token == end_of_text_token) return .done;
        ...
        if (context.generated_tokens_count == runtime.generated_tokens.len) return .done;
...
const RuntimeMemoryLayout = struct {
    ...
    fn init(model: *const model_module, workers_count: usize, config: abi.Config) InitError!RuntimeMemoryLayout {
        ...
        const encoder_shared_scratch_relative = try reserve(f32, &encoder_phase_size, std.math.mul(usize, workers_count, attention.encoderScratchValuesCount(1)) catch return error.MemorySizeOverflow);
        ...
        const lane_float_rows_relative: Slice(f32) = .{
            ...
            .elements_count = std.math.mul(usize, workers_count, encoder_module.laneFloatScratchValuesCount(dimensions)) catch return error.MemorySizeOverflow,
        };
        ...
        const lane_float_size = std.math.mul(usize, lane_float_rows_relative.elements_count, @sizeOf(f32)) catch return error.MemorySizeOverflow;
        const lane_float_end = std.math.add(usize, lane_float_rows_relative.offset, lane_float_size) catch return error.MemorySizeOverflow;
        ...
        const lane_quantized_rows = try reserve(u8, &size, std.math.mul(usize, workers_count, encoder_module.laneQuantizedScratchValuesCount(dimensions)) catch return error.MemorySizeOverflow);
    ...
    fn reserve(comptime Element: type, size: *usize, elements_count: usize) error{MemorySizeOverflow}!Slice(Element) {
        ...
        const rounded = std.math.add(usize, size.*, memory_alignment - 1) catch return error.MemorySizeOverflow;
        const offset = rounded & ~(memory_alignment - 1);
        const bytes_count = std.math.mul(usize, elements_count, @sizeOf(Element)) catch return error.MemorySizeOverflow;
        size.* = std.math.add(usize, offset, bytes_count) catch return error.MemorySizeOverflow;
    ...
    fn Slice(comptime Element: type) type {
        return struct {
            ...
            fn rebase(slice: @This(), base_offset: usize) @This() {
                ...
                return .{
                    .offset = std.math.add(usize, base_offset, slice.offset) catch unreachable,
                    .elements_count = slice.elements_count,
                };
            }

            fn bind(slice: @This(), memory: []align(memory_alignment) u8) []align(memory_alignment) Element {
                const bytes_count = std.math.mul(usize, slice.elements_count, @sizeOf(Element)) catch unreachable;
...
fn transcriptSize(model: *const model_module, tokens_count_max: usize) InitError!usize {
    ...
    if (offsets.len != model_module.dimensions(model.kind).vocabulary_tokens_count + 1 or offsets[0] != 0) return error.InvalidModel;

    var token_size_max: usize = 0;
    for (offsets[0 .. offsets.len - 1], offsets[1..]) |start, end| {
        if (end < start or end > model.vocabulary.bytes.len) return error.InvalidModel;
        token_size_max = @max(token_size_max, end - start);
    }
    if (offsets[offsets.len - 1] != model.vocabulary.bytes.len) return error.InvalidModel;
    if (token_size_max == 0) return error.InvalidModel;
...
test "worker bitset runtime scratch sizing rejects overflow" {
    const allocator = std.testing.allocator;
    const kind: ModelKind = .whisper_base_en;
