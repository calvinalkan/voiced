...
fn enqueue(scheduler: *Scheduler, operation: *Operation) void {
    ...
    operation.state = .{ .queued = if (tail) |last|
        last.state.queued
    else
        operation };

    if (tail) |last| {
        last.state.queued = operation;
    }
...
fn schedule(scheduler: *Scheduler) void {
    ...
    while (scheduler.queue_tail.load(.monotonic)) |tail| {
        ...
        const cancelled = if (operation.cancellation) |flag|
            flag.load(.acquire)
        else
            false;
        ...
        if (!cancelled) {
            for (scheduler.idle_words) |*word| {
                ...
                if (count == operation.workers_count_max) {
                    break;
                }
            }
        ...
        if (operation == tail) {
            scheduler.queue_tail.store(null, .release);
        } else {
            tail.state.queued = operation.state.queued;
        }
...
const Operation = struct {
    ...
    fn checkpoint(operation: *Operation, scheduler: *Scheduler, count: usize) Action {
        ...
        if (arrivals + 1 == count) {
            ...
            const action: Action = if (cancelled)
                .cancelled
            else if (operation.step_result == .done)
                .done
            else if (scheduler.queue_tail.load(.acquire) != null or
                (count < operation.workers_count_max and scheduler.hasIdleWorkers()))
                .yield
            else
                .continue_running;
...
const Mutex = struct {
    ...
    fn lock(mutex: *Mutex) void {
        if (mutex.state.cmpxchgStrong(0, 1, .acquire, .monotonic) == null) {
            return;
        }

        while (mutex.state.swap(2, .acquire) != 0) {
            futexWait(&mutex.state.raw, 2);
        }
    }

    fn unlock(mutex: *Mutex) void {
        if (mutex.state.swap(0, .release) == 2) {
            futexWake(&mutex.state.raw, 1);
        }
    }
};
...
const Layout = struct {
    ...
    fn init(count: usize) InitError!Layout {
        ...
        const words_size = std.math.mul(usize, words_count, @sizeOf(Word)) catch {
            return error.MemorySizeOverflow;
        };

        const assignments_offset = std.math.add(usize, words_offset, words_size) catch {
            return error.MemorySizeOverflow;
        };

        const assignments_size = std.math.mul(usize, count, @sizeOf(Assignment)) catch {
            return error.MemorySizeOverflow;
        };

        const end = std.math.add(usize, assignments_offset, assignments_size) catch {
            return error.MemorySizeOverflow;
        };

        const rounded = std.math.add(usize, end, abi.memory_alignment - 1) catch {
            return error.MemorySizeOverflow;
        };
...
test "worker bitset handles word boundaries, sparse availability and queued cancellation" {
    ...
    for ([_]usize{ 1, 31, 32, 33, 63, 64, 65, 127, 128, 129, 1025 }) |count| {
        ...
        for (scheduler.idle_words) |*word| {
            available += @popCount(word.load(.monotonic));
        }
...
test "worker bitset dispatches across words with real threads" {
    ...
    const Context = struct {
        ...
        fn step(raw: *anyopaque, lane: Lane) Step {
            ...
            if (context.admitted) |admitted| {
                if (lane.isLeader() and admitted.fetchAdd(1, .acq_rel) == 1) {
                    futexWake(&admitted.raw, count);
                }

                var value = admitted.load(.acquire);

                while (value < 2) : (value = admitted.load(.acquire)) {
                    futexWait(&admitted.raw, value);
                }
            }
        ...
        fn run(pool_arg: *Scheduler, context: *@This(), width: usize) void {
            pool_arg.run(width, context, step, null) catch {
                unreachable;
            };
        }
    };
    ...
    for ([_]usize{ 1, 32, 33, 64, 65 }) |width| {
        ...
        for (context.seen, 0..) |seen, index| {
            try std.testing.expectEqual(index < width, seen);
        }
    }
    ...
    for (first_context.seen, 0..) |seen, index| {
        try std.testing.expectEqual(index < 33, seen);
    }

    for (second_context.seen, 0..) |seen, index| {
        try std.testing.expectEqual(index < 32, seen);
    }
}
...
pub const TestPool = struct {
    ...
    pub fn init(pool: *TestPool, count: usize) !void {
        ...
        errdefer {
            ...
            for (pool.threads[0..started]) |thread| {
                thread.join();
            }
        }
    ...
    pub fn run(pool: *TestPool, comptime function: anytype, arguments: anytype) void {
        ...
        pool.scheduler.run(pool.threads.len, &context, Callback.step, null) catch {
            unreachable;
        };
    }
};
