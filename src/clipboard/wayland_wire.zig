...
pub const Connection = struct {
    ...
    pub fn deinit(self: *Connection) void {
        ...
        for (self.fds[0..self.fds_count]) |fd| {
            _ = linux.close(fd);
        }

        if (self.output_fd) |pending| {
    ...
    pub fn initEmpty(self: *Connection) void {
        ...
        inline for (std.meta.fields(Connection)) |field| {
            if (comptime std.mem.eql(u8, field.name, "input") or std.mem.eql(u8, field.name, "output") or std.mem.eql(u8, field.name, "fds")) {
                continue;
            }
    ...
    pub fn send(self: *Connection, object: u32, opcode: u16, body: []const u8, fd: ?i32) Error!void {
        if (body.len % 4 != 0 or body.len + 8 > 65532) {
            return error.InvalidMessage;
        }

        if (fd != null and self.output_fd != null) {
        ...
        if (self.output_sent > 0) {
            ...
            if (self.output_fd) |*pending| {
                pending.offset -= self.output_sent;
            }

            self.output_sent = 0;
        }

        if (body.len + 8 > self.output.len - self.output_size) {
            return error.QueueFull;
        }

        if (fd) |value| {
            self.output_fd = .{ .fd = value, .offset = self.output_size };
        }
    ...
    pub fn flush(self: *Connection) Error!void {
        ...
        const end = if (self.output_fd) |pending|
            if (pending.offset > self.output_sent) pending.offset else self.output_size
        else
            self.output_size;
        ...
        const with_fd = if (self.output_fd) |pending|
            pending.offset == self.output_sent
        else
            false;
    ...
    pub fn receive(self: *Connection) Error!?Message {
        const target: usize = if (self.input_size < 8)
            8
        else
            std.mem.readInt(u32, self.input[4..8], endian) >> 16;
        ...
        if (self.input_size < target) {
            ...
            while (msg.controllen - cursor >= @sizeOf(linux.cmsghdr)) {
                ...
                if (header.len < @sizeOf(linux.cmsghdr) or header.len > msg.controllen - cursor) {
                    return error.InvalidMessage;
                }
            ...
            if (target == 8 or self.input_size < target) {
                return null;
            }
        }
...
pub const Reader = struct {
    ...
    pub fn string(self: *Reader) Error![]const u8 {
        ...
        if (bytes[bytes.len - 1] != 0 or std.mem.indexOfScalar(u8, bytes[0 .. bytes.len - 1], 0) != null) {
            return error.InvalidMessage;
        }
...
pub const Writer = struct {
    ...
    pub fn string(self: *Writer, value: []const u8) Error!void {
        ...
        if (length + 4 > self.bytes.len - self.size) {
            return error.QueueFull;
        }
