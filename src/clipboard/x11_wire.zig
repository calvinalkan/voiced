...
pub const Connection = struct {
    ...
    pub fn initEmpty(self: *Connection) void {
        inline for (std.meta.fields(Connection)) |field| {
            if (comptime std.mem.eql(u8, field.name, "input") or std.mem.eql(u8, field.name, "output")) {
                continue;
            }
    ...
    pub fn connectDisplay(self: *Connection, display: []const u8, authority: ?[]const u8, home: ?[]const u8) Error!void {
        ...
        const path = std.fmt.bufPrint(&socket_path, "/tmp/.X11-unix/X{d}", .{display_number}) catch {
    ...
    pub fn flush(self: *Connection) Error!void {
        ...
        if (self.errno == .AGAIN or self.errno == .INTR) {
            return;
        }
    ...
    pub fn sendParts(self: *Connection, opcode: u8, data: u8, body: []const u8, trailing: []const u8) Error!u16 {
        ...
        if (request_size > std.math.maxInt(u16) * 4) {
            return error.InvalidMessage;
        }

        if ((self.maximum_request_size != 0 and request_size > self.maximum_request_size) or request_size > self.output.len - self.output_size) {
    ...
    pub fn receiveSetup(self: *Connection) Error!?Setup {
        ...
        if (!try self.readTo(target)) {
            return null;
        }

        const status = self.input[0];
        if (status != 1) {
            if (status == 0 and self.input[1] > target - 8) {
                return error.InvalidMessage;
            }

            const reason_size = if (status == 0)
                @as(usize, self.input[1])
            else
                target - 8;
        ...
        if (target < 40 or std.mem.readInt(u16, self.input[2..4], endian) != 11 or std.mem.readInt(u16, self.input[4..6], endian) != 0) {
            return error.InvalidMessage;
        }
    ...
    fn readCookie(self: *Connection, display_number: u16, authority: ?[]const u8, home: ?[]const u8, cookie: *[16]u8) Error!bool {
        ...
        const path = authority orelse
            if (home) |directory|
                std.fmt.bufPrint(&default_path, "{s}/.Xauthority", .{directory}) catch {
        ...
        const display = std.fmt.bufPrint(&display_buffer, "{d}", .{display_number}) catch {
            unreachable;
        };
        ...
        while (try reader.readU16OrEof()) |family| {
            ...
            if (family_matches and address_matches and number_matches and name_field.complete and std.mem.eql(u8, name_field.bytes, "MIT-MAGIC-COOKIE-1")) {
                if (!data_field.complete or data_field.bytes.len != cookie.len) {
                    return error.InvalidAuthority;
                }
...
const AuthorityReader = struct {
    ...
    fn readField(self: *AuthorityReader, storage: []u8) Error!struct { bytes: []const u8, complete: bool } {
        const size = (try self.readU16OrEof()) orelse {
            return error.InvalidAuthority;
        };
...
fn parseDisplay(display: []const u8) Error!u16 {
    ...
    if (end == 0 or (end != suffix.len and !std.mem.eql(u8, suffix[end..], ".0"))) {
        return error.UnsupportedDisplay;
    }
