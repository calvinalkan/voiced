    ...
    inline for (std.meta.fields(WaylandClipboard)) |field| {
        if (comptime std.mem.eql(u8, field.name, "connection")) {
            ...
            @field(self, field.name) = comptime field.defaultValue() orelse
                @compileError("WaylandClipboard metadata requires a default: " ++ field.name);
        }
    }
...
pub fn deinit(self: *WaylandClipboard) void {
    for (0..self.transfers.len) |index| {
...
pub fn publish(self: *WaylandClipboard, publication: PublicationId, text: []const u8, now_ns: u64) Result(void) {
    ...
    self.publishInternal(slot, publication, text, now_ns) catch |err| {
        return .{ .err = self.failure(err) };
    };
...
fn start(self: *WaylandClipboard, environment: Environment) !void {
    ...
    const socket = if (std.fs.path.isAbsolute(display))
        display
    else
        try std.fmt.bufPrint(&path, "{s}/{s}", .{ environment.runtime_directory orelse {
...
fn advanceInternal(self: *WaylandClipboard, now_ns: u64) !Event {
    ...
    for (0..64) |_| {
        const msg = try self.connection.receive() orelse {
            if (self.connection.errno == .AGAIN or self.connection.errno == .INTR) {
                break;
            }
    ...
    for (&self.sources, 0..) |*source, index| {
        ...
        const borrowed = for (self.transfers) |entry| {
            if (entry != null and entry.?.source == index) {
                break true;
            }
        } else false;
    ...
    try self.watch(self.connection.fd, linux.EPOLL.CTL_MOD, linux.EPOLL.IN | (if (self.connection.output_size > 0)
...
fn dispatch(self: *WaylandClipboard, object: u32, opcode: u16, r: *wire.Reader, now_ns: u64, event: *Event) !void {
    ...
    const entry = self.objects[object] orelse {
        return error.InvalidMessage;
    };
    ...
    switch (entry.kind) {
        .display => switch (opcode) {
            ...
            1 => {
                const id = try r.word();
                if (id < 2 or id >= self.objects.len or self.objects[id] == null) {
                    return error.InvalidMessage;
                }
        ...
        .registry => switch (opcode) {
            ...
            1 => {
                const name = try r.word();
                if (name == self.seat.name or name == self.manager().name) {
                    return error.GlobalRemoved;
                }
            },
            else => return error.InvalidMessage,
        },
        ...
        .keyboard => switch (opcode) {
            ...
            1 => {
                ...
                if (self.phase == .focus and surface == self.phase.focus.popup.surface) {
                    try self.setSelection(serial);
                }
            },
        ...
        .source => {
            ...
            if (opcode == send_opcode) {
                ...
                const supported = for (mime_types) |known| {
                    if (std.mem.eql(u8, mime, known)) {
                        break true;
                    }
                } else false;

                const source: ?u1 = for (self.sources, 0..) |s, index| {
                    if (s == .offered and s.offered.object == object) {
                        break @intCast(index);
                    }
                } else null;

                if (!supported or source == null or entry.destroyed) {
                    return;
                }
...
fn bindGlobals(self: *WaylandClipboard) !void {
    ...
    self.mode = switch (self.protocol_policy) {
        .best => if (self.ext.name != 0)
            .ext
        else if (self.wlr.name != 0)
            .wlr
        else
            .core,
    ...
    if (self.mode == .core) {
        ...
        if (self.gtk.version >= 4) {
            try self.bind(&self.gtk, "gtk_shell1", 4, .gtk_shell);
        }
...
fn createPopup(self: *WaylandClipboard) !Popup {
    ...
    const gtk = if (self.gtk.object != 0)
        try self.allocate(.gtk_surface)
    else
        0;
    ...
    if (gtk != 0) {
        try self.words(self.gtk.object, 0, &.{ gtk, surface });
    }
...
fn setSelection(self: *WaylandClipboard, serial: u32) !void {
    const pending = self.pendingCopy() orelse {
        return error.InvalidMessage;
    };
    ...
    if (self.mode == .core) {
        try self.words(self.device, 1, &.{ source, serial });
    } else {
        try self.words(self.device, 0, &.{source});
    }
...
fn acquired(self: *WaylandClipboard, event: *Event) !void {
    const slot = (self.pendingCopy() orelse {
        return error.InvalidMessage;
    }).slot;
...
fn popupView(self: *WaylandClipboard) ?*Popup {
    return switch (self.phase) {
        ...
        .selection => |*selection| if (selection.popup) |*value|
            value
        else
            null,
        else => null,
...
fn failure(self: *WaylandClipboard, cause: anyerror) Error {
    const problem = self.problem orelse
        Error{ .transport = .{ .cause = cause, .errno = self.connection.errno, .object = self.last_object, .opcode = self.last_opcode } };
