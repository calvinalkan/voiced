...
pub const Resampler = struct {
    ...
    pub fn configure(self: *Resampler, rate: u32) Error!void {
        ...
        const rows: usize = if (rate % output_rate == 0)
            1
        else
            phases + 1;
        ...
        for (0..rows) |phase| {
            ...
            for (row[0..taps], 0..) |*coefficient, tap| {
                ...
                const sinc = if (@abs(argument) < 1e-12)
                    cutoff
                else
                    cutoff * @sin(argument) / argument;
            ...
            for (row[0..taps]) |*coefficient| {
                coefficient.* /= @floatCast(sum);
            }
        }
    }
    ...
    pub fn process(self: *Resampler, input: anytype, output: []f32) Error![]f32 {
        ...
        if (self.input_rate == output_rate) {
            if (input_count > output.len) {
                return error.OutputFull;
            }

            for (0..input_count) |index| {
                const sample = if (slice)
                    input[index]
                else
                    input.sample(index) catch {
