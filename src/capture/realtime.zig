...
const RealtimeSchedulingUnavailableFields = logging.FieldSet(.{
    .@"error",
    .system_error,
    .bus_error,
    .detail,
});
const RealtimeSchedulingUnavailable = log.Event(.realtime_scheduling_unavailable, RealtimeSchedulingUnavailableFields);
...
pub fn acquire(recording_id: u64, address: []const u8, control_fd: linux.fd_t) void {
    ...
    if (linux.errno(linux.sched_setscheduler(0, .{ .mode = .FIFO, .RESET_ON_FORK = true }, &params)) == .SUCCESS) {
        return;
    }
    ...
    request.run(address) catch |err| {
        RealtimeSchedulingUnavailable.emit(.warn, .{ .recording_id = recording_id }, .{
            .@"error" = @errorName(err),
            .system_error = request.connection.errno,
            .bus_error = request.error_name[0..request.error_name_size],
            .detail = request.error_message[0..request.error_message_size],
        });
    };
}
...
const Request = struct {
    ...
    fn run(self: *Request, address_text: []const u8) !void {
        ...
        while (true) {
            ...
            if (std.mem.indexOf(u8, input, "\r\n")) |end| {
                if (!std.mem.startsWith(u8, input[0..end], "OK ")) {
        ...
        while (self.connection.output_size > 0) {
            try self.progress();
        }
        ...
        if (priority <= 0 or priority > 99 or time_limit <= 0) {
    ...
    fn property(self: *Request, name: []const u8) !i64 {
        ...
        if (!std.mem.eql(u8, message.signature, "v")) {
            return error.InvalidRealtimeLimits;
        }

        const signature = try message.body.signature();

        const value: i64 = if (std.mem.eql(u8, signature, "i"))
            @as(i32, @bitCast(try message.body.uint32()))
        else if (std.mem.eql(u8, signature, "x"))
            @bitCast(try message.body.uint64())
        else
            return error.InvalidRealtimeLimits;
        ...
        while (true) {
            if (try dbus.Message.parse(self.connection.input[0..self.connection.input_size])) |message| {
                ...
                if (now() >= self.deadline) {
                    return error.RealtimeRequestTimedOut;
                }
    ...
    fn progress(self: *Request) !void {
        ...
        var fds = [_]linux.pollfd{
            ...
            .{ .fd = self.connection.fd.?, .events = linux.POLL.IN | (if (self.connection.output_size > 0)
        ...
        if (linux.errno(result) != .SUCCESS) {
            return error.RealtimePollFailed;
        }

        if (fds[0].revents != 0) {
            return error.CaptureStopPending;
        }

        if (fds[1].revents & (linux.POLL.ERR | linux.POLL.HUP) != 0) {
            return error.Disconnected;
        }

        if (fds[1].revents & linux.POLL.IN != 0) {
            _ = try self.connection.read();
        }
    }
};
fn now() u64 {
