...
pub fn acquire(recording_id: u64, address: []const u8, control_fd: linux.fd_t) void {
    ...
    request.run(address) catch |err| {
        log.event(.warn, .{ .recording_id = recording_id }, "realtime_scheduling_unavailable", &.{
            .{ "operation", .{ .verbatim = @errorName(err) } },
            .{ "system_error", .{ .errno = request.connection.errno } },
            .{ "bus_error", .{ .str = request.error_name[0..request.error_name_size] } },
            .{ "detail", .{ .str = request.error_message[0..request.error_message_size] } },
        });
    };
}
