        ...
        while (fields.next()) |field| {
            ...
            while (i < field.len) {
                ...
                if (byte == '%') {
                    ...
                    byte = std.fmt.parseInt(u8, field[i..][0..2], 16) catch {
                        return error.InvalidAddress;
                    };
                    i += 2;
                }
                ...
                if (used == result.value.path.len - @as(usize, if (abstract) 0 else 1)) {
            ...
            if (used == start or (!abstract and result.value.path[0] != '/')) {
                return error.InvalidAddress;
            }
...
pub const Connection = struct {
    ...
    pub fn connect(self: *Connection, address: *const Address) Error!void {
        ...
        if (self.errno != .SUCCESS and self.errno != .INPROGRESS) {
...
pub const Message = struct {
    ...
    pub fn parse(bytes: []const u8) Error!?Message {
        ...
        if (fields_size > input_bytes_max - 16 or body_size > input_bytes_max) {
            return error.MessageTooLarge;
        }
        ...
        var message: Message = .{
            .kind = std.enums.fromInt(@FieldType(Message, "kind"), bytes[1]) orelse {
                return error.InvalidMessage;
            },
        ...
        while (reader.position < reader.bytes.len) {
            ...
            if (code <= 9) {
                ...
                if (!std.mem.eql(u8, sig, expected)) {
                    return error.InvalidMessage;
                }
            }
        ...
        if (body_size != 0 and message.signature.len == 0) {
            return error.InvalidMessage;
        }
...
pub const Writer = struct {
    ...
    pub fn call(bytes: []u8, serial: u32, destination: []const u8, path: []const u8, interface: []const u8, member: []const u8, body_signature: []const u8) Error!Writer {
        ...
        if (body_signature.len != 0) {
            try writer.field(8, "g", body_signature);
        }
    ...
    pub fn string(self: *Writer, value: []const u8) Error!void {
        try self.uint32(std.math.cast(u32, value.len) orelse {
    ...
    pub fn stringParts(self: *Writer, parts: []const []const u8) Error!void {
        ...
        for (parts) |part| {
            size = std.math.add(usize, size, part.len) catch {
                return error.BufferFull;
            };
        }

        try self.uint32(std.math.cast(u32, size) orelse {
            return error.BufferFull;
        });
    ...
    fn signature(self: *Writer, value: []const u8) Error!void {
        try self.append(&.{std.math.cast(u8, value.len) orelse {
            return error.BufferFull;
        }});
    ...
    fn append(self: *Writer, bytes: []const u8) Error!void {
        if (bytes.len > self.bytes.len - self.position) {
            return error.BufferFull;
        }
...
pub const Reader = struct {
    ...
    fn text(self: *Reader, size: usize) Error![]const u8 {
        ...
        if ((try self.take(1))[0] != 0 or std.mem.indexOfScalar(u8, value, 0) != null or !std.unicode.utf8ValidateSlice(value)) {
            return error.InvalidMessage;
        }
    ...
    fn skipValue(self: *Reader, sig: []const u8, position: *usize, depth: u8) Error!void {
        if (depth >= 32 or position.* >= sig.len) {
            return error.InvalidMessage;
        }
        ...
        switch (code) {
            ...
            '(', '{' => {
                ...
                while (position.* < sig.len and sig[position.*] != close) {
                    try self.skipValue(sig, position, depth + 1);
                }

                if (position.* == start or position.* == sig.len) {
                    return error.InvalidMessage;
                }
