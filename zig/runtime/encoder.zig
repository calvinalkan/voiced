...
pub const Encoder = struct {
    ...
    pub fn init(memory: Memory, specification: ModelSpecification, workers_count: usize) Encoder {
        ...
        assert(memory.activation.len == positions_count_max * width);
        assert(memory.shared_scratch.len == sharedScratchValuesCount(workers_count));
        assert(memory.query_key_value.len == attention.encoderQueryKeyValueValuesCount(positions_count_max, width));
    ...
    pub fn encode(encoder: *Encoder, specification: ModelSpecification, weights: *const InferenceWeights, features: log_mel.Features, lane: Lane) void {
        ...
        const activation = encoder.memory.activation[0 .. positions_count * width];
        const query_key_value_storage = encoder.memory.query_key_value[0..attention.encoderQueryKeyValueValuesCount(positions_count, width)];
        ...
        const attention_scratch_values_count = attention.encoderScratchValuesCount(lane.count);
...
pub fn sharedScratchValuesCount(workers_count: usize) usize {
    return attention.encoderScratchValuesCount(workers_count);
}
...
pub fn laneFloatScratchValuesCount(specification: ModelSpecification) usize {
    ...
    const convolution_values_count = (convolution_rows_count + convolution_kernel_width) * specification.encoder_width;

    return @max(convolution_values_count, linear.ffn_rows_per_tile * specification.encoder_ffn_width);
}
...
pub fn laneQuantizedScratchValuesCount(specification: ModelSpecification) usize {
    ...
    const second_convolution_values_count = convolution_positions_per_tile * specification.encoder_width * convolution_kernel_width;
    ...
    const projection_values_count = linear.rows_per_tile * specification.encoder_width;
    const ffn_values_count = linear.ffn_rows_per_tile * @max(specification.encoder_width, specification.encoder_ffn_width);
...
/// functions. Executor lanes synchronize before claiming tiles; no intermediate
