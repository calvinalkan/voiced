...
pub const Connection = struct {
    ...
    pub fn connect(self: *Connection, path: []const u8) Error!void {
        ...
        if (path.len == 0 or path.len >= address.path.len or std.mem.indexOfScalar(u8, path, 0) != null) return error.ConnectFailed;
        ...
        if (self.errno != .SUCCESS) return error.ConnectFailed;
    }

    pub fn deinit(self: *Connection) void {
        for (self.descriptors[0..self.descriptors_count]) |fd| _ = linux.close(fd);
    ...
    pub fn send(self: *Connection, object: u32, opcode: u8, payload: []const u8) Error!void {
        ...
        if (16 + payload.len > self.output.len - self.output_size) return error.QueueFull;
    ...
    pub fn flush(self: *Connection) Error!void {
        if (self.output_sent == self.output_size) return;
    ...
    pub fn receive(self: *Connection) Error!?Message {
        ...
        if (self.input_size >= 16) {
            ...
            if (target > self.input.len) return error.MessageTooLarge;
            if (std.mem.readInt(u32, self.input[12..16], endian) > descriptors_max) return error.TooManyDescriptors;
        }
        if (self.input_size < target) {
            ...
            while (message.controllen - cursor >= @sizeOf(linux.cmsghdr)) {
                ...
                if (header.len < @sizeOf(linux.cmsghdr) or header.len > message.controllen - cursor) return error.InvalidMessage;
            ...
            if (overflow or message.flags & linux.MSG.CTRUNC != 0) return error.TooManyDescriptors;
            if (result == 0) return error.Disconnected;
            self.input_size += result;
            if (target == 16 or self.input_size < target) return null;
        }
        if (self.descriptors_count < std.mem.readInt(u32, self.input[12..16], endian)) return error.InvalidMessage;
        ...
        while (pods.bytes.len > 0) _ = try pods.next();
    ...
    pub fn duplicateDescriptor(self: *Connection, index: i64) Error!linux.fd_t {
        if (index < 0 or index >= std.mem.readInt(u32, self.input[12..16], endian)) return error.InvalidMessage;
        const result = linux.fcntl(self.descriptors[@intCast(index)], linux.F.DUPFD_CLOEXEC, 0);
        self.errno = linux.errno(result);
        if (self.errno != .SUCCESS) return error.DescriptorFailed;
    ...
    pub fn consume(self: *Connection) void {
        ...
        for (self.descriptors[0..count]) |fd| _ = linux.close(fd);
...
pub const Pod = struct {
    ...
    pub fn scalar(self: Pod, kind: u32) Error!u32 {
        ...
        if (self.kind == 19) {
            if (self.body.len != 20 or std.mem.readInt(u32, self.body[0..4], endian) != 0 or std.mem.readInt(u32, self.body[8..12], endian) != 4 or std.mem.readInt(u32, self.body[12..16], endian) != kind) return error.InvalidMessage;
            return std.mem.readInt(u32, self.body[16..20], endian);
        }
        if (self.kind != kind or self.body.len != 4) return error.InvalidMessage;
...
pub const Reader = struct {
    ...
    pub fn structure(self: *Reader) Error!Reader {
        ...
        if (p.kind != 14) return error.InvalidMessage;
        return .{ .bytes = p.body };
    }
    ...
    pub fn long(self: *Reader, kind: u32) Error!i64 {
        const p = try self.next();
        if (p.kind != kind or p.body.len != 8) return error.InvalidMessage;
        return std.mem.readInt(i64, p.body[0..8], endian);
    }
    pub fn string(self: *Reader) Error![]const u8 {
        const p = try self.next();
        if (p.kind != 8 or p.body.len == 0 or p.body[p.body.len - 1] != 0) return error.InvalidMessage;
        const value = p.body[0 .. p.body.len - 1];
        if (std.mem.indexOfScalar(u8, value, 0) != null or !std.unicode.utf8ValidateSlice(value)) return error.InvalidMessage;
        return value;
    }
