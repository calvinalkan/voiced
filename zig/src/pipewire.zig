...
fn runCapture(launch: Launch, client: *native.Client, report: *Report) !CaptureResult {
    ...
    while (outcome == .none) {
        ...
        if (fds[2].revents & linux.POLL.IN == 0) continue;
        {
            ...
            if (count > audio_exchange.slot_samples_capacity - capture.active_slot.?.samples_count) {
...
fn publishCompleteBlock(
    ...
    assert(block_samples_count <= audio_exchange.slot_samples_capacity);
    if (realtime.active_slot) |active_slot| {
        assert(active_slot.samples_count <= audio_exchange.slot_samples_capacity);
    }
...
fn beginNextAvailableSlot(realtime: *RealtimeCapture) bool {
    ...
    const slot_index = audio_exchange.SlotIndex.fromPublicationOrdinal(
