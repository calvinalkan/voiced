...
pub const Error = union(enum) {
    ...
    unsupported: enum { display, seat, keyboard, clipboard, surface },
    timed_out: Phase,
    selection_lost,
    busy,
    invalid_text,
...
pub const Client = struct {
    ...
    fn bindGlobals(self: *Client) !void {
        ...
        self.mode = if (!self.fallback_only and self.ext.name != 0) .ext else if (!self.fallback_only and self.wlr.name != 0) .wlr else .core;
    ...
    fn allocate(self: *Client, kind: Kind) !u32 {
    ...
    fn destroy(self: *Client, object: u32, opcode: u16) !void {
        try self.words(object, opcode, &.{});
        self.objects[object].?.destroyed = true;
    }
    fn sync(self: *Client, kind: Kind) !void {
        try self.words(1, 0, &.{try self.allocate(kind)});
    }
    fn words(self: *Client, object: u32, opcode: u16, args: []const u32) !void {
        var w: wire.Writer = .{};
        for (args) |arg| try w.word(arg);
        try self.connection.send(object, opcode, w.data(), null);
    }
    fn watch(self: *Client, fd: i32, operation: u32, events: u32) !void {
    ...
    fn closeTransfer(self: *Client, index: usize) void {
