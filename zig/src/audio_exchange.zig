...
pub const AudioExchange = struct {
...
pub fn initialize(exchange: *AudioExchange) void {
    ...
    for (&exchange.slots) |*slot| slot.published_samples_count_atomic = 0;
}
...
pub fn publishAudioCallbacksCount(exchange: *AudioExchange, callbacks_count: u64) void {
    ...
    @atomicStore(u64, &exchange.audio_callbacks_count_atomic, callbacks_count, .release);
}

pub fn acquireAudioCallbacksCount(exchange: *const AudioExchange) u64 {
    return @atomicLoad(u64, &exchange.audio_callbacks_count_atomic, .acquire);
}
...
pub fn tryAcquireWriter(slot: *AudioSlot) ?SlotWriter {
    if (@atomicLoad(u32, &slot.published_samples_count_atomic, .acquire) != 0) return null;
    return .{ .slot = slot };
}
...
pub fn releaseConsumedSlot(slot: *AudioSlot) void {
    ...
    assert(samples_count <= slot_samples_capacity);
    @atomicStore(u32, &slot.published_samples_count_atomic, 0, .release);
}
