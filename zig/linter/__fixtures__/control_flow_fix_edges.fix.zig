...
fn errorFallback(fail: bool) u8 {
    return attempt(fail) catch |err|
        recover(err);
}

fn errorExit(fail: bool) u8 {
    return attempt(fail) catch {
        return 9;
    };
}
...
fn commentedValue(flag: bool) u8 {
    return if (flag)
        identity(1) // Keep the then comment.
    else
        identity(2); // Keep the else comment.
}

fn nestedValue(first: bool, second: bool) u8 {
    return if (first)
        (if (second) identity(1) else identity(2))
    else
        identity(3);
}
...
fn orderAndTry(value: *u8, fail: bool) error{Failed}!u8 {
    return step(value, 1) +
        try failingStep(value, 2, fail) + step(value, 3);
}
...
fn hiddenBreak() u8 {
    return result: {
    break :result 2; }; }
fn hiddenContinue() void { for (0..1) |_| {
    continue; } }
fn hiddenUnreachable() noreturn {
    unreachable; }
...
test "control flow preserves values, effects, and cleanup" {
    ...
    try std.testing.expectEqual(@as(u8, 6),
        try orderAndTry(&value, false));
    ...
    try std.testing.expectEqual(@as(u8, 12), value);
    try std.testing.expectEqual(@as(u8, 10),
        try aggregateTry(false));
