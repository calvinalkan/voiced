...
fn inlineCleanup(counter: *u8) void { const resource: Resource = .{ .counter = counter };
    defer resource.deinit();

    increment(counter); }
fn inlineCategories(counter: *u8) u8 { increment(counter);

    const value: u8 = 3;
return value; }
fn inlineBlocks(flag: bool) void { switch (flag) { true => {},
