...
with tempfile.TemporaryDirectory(prefix="run-", dir=experiment) as temporary:
    root = Path(temporary)
    env = dict(os.environ, VOICED_INSTANCE="test", XDG_RUNTIME_DIR=str(root),
               XDG_CONFIG_HOME=str(root / "user-config"), XDG_STATE_HOME=str(root / "state"),
               DBUS_SESSION_BUS_ADDRESS="unix:path=" + str(root / "missing-bus"),
...
''')
    ...
    with service("--transcript-output", "stdout") as daemon:
        ...
        wait_until(lambda: len(children(daemon.pid)) == 1)
        capture = next(iter(children(daemon.pid)))
        assert Path(f"/proc/{capture}/comm").read_text().strip() == "voiced-capture"
        cli("stop")
        assert cli("status")["phase"] == "idle"
    ...
    print("Native notifications, keyboard and configuration integration passed.")
PY
    python3 zig/scripts/test-native-output.py
    exit 0
fi

echo "Usage: $0 --zig-control | --zig-logging | --zig-output | --zig-replay" >&2
exit 2
